...
## 架构
- `app.js`（也是 `bin`）——服务入口。执行 `generateConfig()` 预热运行时凭证（anonymous_token 与 xeapi 公钥，存于内存），再调用 `server.serveNcmApi()`。不依赖 `/tmp` 等可写文件系统。
...
- `util/request.js`——唯一的对外 HTTP 层（axios）。按 `crypto`（`api`/`eapi`/`weapi`/`linuxapi`/`xeapi`）加密并设置 IP 头；运行时凭证一律从 `util/runtimeState.js` 读取，不再有任何 require 期文件 I/O。
- `util/runtimeState.js`——统一的进程内运行时状态（`anonymous_token` 与 `xeapi_public_key`）。默认纯内存、零文件 I/O；通过 `loadSeed()` 可选地从环境变量 `NETEASE_ANONYMOUS_TOKEN` / `NETEASE_XEAPI_PUBLIC_KEY` 或旧 `/tmp` 文件加载迁移种子。`util/credentials.js` 是唯一的网络刷新层（含并发去重与防重入，适配 serverless 冷启动）。
...
## 测试
- `pnpm test` 跑 `server.test.js` + `main.test.js` + `util/runtimeState.test.js`。`server.test.js` 在 `before()` 里启动真实服务器，`test/*.test.js` 全部请求**真实网易云 API**——必须联网，且可能因上游风控/限流偶发失败。`main.test.js` 与 `util/runtimeState.test.js` 是纯单测，不需要网络。
...
- 只跑离线单测：`pnpm exec mocha -r intelli-espower-loader -t 60000 main.test.js util/runtimeState.test.js --exit`
...
- **运行时凭证在内存中**：`anonymous_token`、`xeapi_public_key` 存于 `util/runtimeState.js`（进程内），不再写 `/tmp`，可在只读文件系统 / Android / iOS / serverless 上运行。缺失时由 `util/credentials.js` 按需刷新；旧 `/tmp` 文件仅作为可选迁移种子（`loadSeed()`），环境变量 `NETEASE_ANONYMOUS_TOKEN` / `NETEASE_XEAPI_PUBLIC_KEY` 优先。重启服务（或调用 `generateConfig()`）会重新刷新。
